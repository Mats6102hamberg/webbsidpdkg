  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main className="mx-auto flex max-w-2xl flex-col gap-4 px-6 py-12">
        <h1 className="text-2xl font-semibold">{translate("checkout.successTitle")}</h1>
        <p className="text-slate-600">{translate("checkout.successBody")}</p>
        <Link
          className="w-fit rounded bg-slate-900 px-4 py-2 text-sm font-medium text-white"
          href={`/${locale}/books/petanque-dkg`}
        >
          {translate("checkout.backToBook")}
        </Link>
      </main>
    </div>
  );
